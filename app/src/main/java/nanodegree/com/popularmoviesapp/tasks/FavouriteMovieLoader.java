    ...
    public ArrayList< MovieData > loadInBackground() {
        ...
        try {
            ...
            while ( cursor.moveToNext() ) {
                ...
                movieData.setAdult( (cursor.getInt( cursor.getColumnIndex
                        ( MoviesContract.MovieEntry.IS_ADULT ) ) == 1) );
                movieData.setVideo( (cursor.getInt( cursor.getColumnIndex
                        ( MoviesContract.MovieEntry.VIDEO ) ) == 1) );
                movieData.setBackdropPath( cursor.getString( cursor.getColumnIndex
                        ( MoviesContract.MovieEntry.BACKDROP_PATH ) ) );
                movieData.setOriginalLanguage( cursor.getString( cursor.getColumnIndex
                        ( MoviesContract.MovieEntry.ORIGINAL_LANGUAGE ) ) );
                movieData.setOriginalTitle( cursor.getString( cursor.getColumnIndex
                        ( MoviesContract.MovieEntry.ORIGINAL_TITLE ) ) );
                movieData.setTitle( cursor.getString( cursor.getColumnIndex
                        ( MoviesContract.MovieEntry.TITLE ) ) );
                movieData.setOverview( cursor.getString( cursor.getColumnIndex
                        ( MoviesContract.MovieEntry.OVERVIEW ) ) );
                movieData.setPosterPath( cursor.getString( cursor.getColumnIndex
                        ( MoviesContract.MovieEntry.POSTER_PATH ) ) );
                movieData.setReleaseDate( cursor.getString( cursor.getColumnIndex
                        ( MoviesContract.MovieEntry.RELEASE_DATE ) ) );
                movieData.setVoteCount( cursor.getInt( cursor.getColumnIndex
                        ( MoviesContract.MovieEntry.VOTE_COUNT ) ) );
                movieData.setId( Integer.parseInt( cursor.getString( cursor.getColumnIndex
                        ( MoviesContract.MovieEntry.MOVIES_ID ) ) ) );
                movieData.setPopularity( Double.parseDouble( cursor.getString( cursor.getColumnIndex
                        ( MoviesContract.MovieEntry.MOVIES_ID ) ) ) );
                movieData.setVoteAverage( Double.parseDouble( cursor.getString( cursor.getColumnIndex
